...
export default function CourseDetailPage() {
  ...
  useEffect(() => {
    const fetchCourseData = async () => {
      try {
        ...
        // Filter out duplicate distributions based on term
        if (courseData.distributions && courseData.distributions.length > 0) {
          // Use a Map to keep only unique term entries
          const uniqueDistributions = Array.from(
            new Map(courseData.distributions.map(dist => [dist.term, dist]))
            .values()
          );
          
          // Update the course data with filtered distributions
  ...
  const gradeDistributionData = selectedDistribution 
    ? GRADE_LABELS.map((grade, index) => {
        // Log the grade counts for debugging
        console.log(`Term ${selectedTerm} - Grade ${grade}: ${selectedDistribution.grade_counts[index] || 0}%`);
  ...
  const createDistributionBreakdown = (distribution: any) => {
    ...
    // Log the distribution data for debugging
    console.log(`Creating breakdown for term: ${distribution.term}`);
    console.log(`Grade counts:`, distribution.grade_counts);
